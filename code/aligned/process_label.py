...
class ProcessLabel:
    ...
    def to_text(cls, labels:list):
        ...
        if labels.count(1) == 0:
            result.append('Excellent job! Keep it up, yogi! Wanna try different pose?')

        if labels.count(1) == 1:
            index = np.where(np.array(labels) == 1)[0]
            output = trans_fd[index[0]]
            result.append(f'Very very nice! One little thing: try to {output}!')

        if labels.count(1) >= 2 and labels.count(1) < 7:
            index = np.where(np.array(labels) == 1)[0] #list
            result.append('Well done! Couple of things to keep in mind for you:')
            for i in index:
                result.append(f'- {trans_fd[i][0].capitalize() +trans_fd[i][1:]}.')

        if labels.count(1) >= 7:
            result.append("Are you sure you were following my instructions? Let's try again!")
